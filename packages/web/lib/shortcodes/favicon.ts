...
async function faviconShortcode(src: string) {
  const props = {
    ...
    filenameFormat: (_hash: string, _src: string, width: number, format: string) => {
      return `favicon-${width}.${format}`;
    },
  };

  const metadata = await Image(src, props);
  const faviconHTML = metadata[FAVICON_FORMAT].map((image: { width: number; url: string }) => {
