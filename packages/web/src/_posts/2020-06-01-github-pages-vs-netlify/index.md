...
description: GitHub Pages is a good option for hosting simple sites for free, but Netlify offers plenty of more advanced features.
...
![Sample output from a local Netlify build running on my site. Netlify steps are listed in bright cyan font against a black background, with numbered steps and information about my Netlify configuration.](./images/local-build.png)
...
![The footer section of a GitHub pull request shows a checklist of Netlify checks that passed or failed during the build process. There are metrics for each step, like the time it took to finish, as well as links to deploy previews.](./images/deploy-preview.png)
...
![The Netlify dashboard for my personal website, showing a list of recent deployments and their git hash IDs.](./images/dashboard.png)
...
![A list of recent deployments in my Netlify dashboard, along with the time it took each one to deploy from start to finish.](./images/faster-deploys.png)
...
![Netlify's dashboard shows billing information for things such as data usage and build minutes used.](./images/billing.png)
...
![Migrating my old search engine results to my new domain using Google Search Console's change of address tool.](./images/migration.png)
