...
You may not have considered how aspect ratios impact web performance, but they're _really_ important. Aspect ratios allow us to size images properly so that they don't cause layout shifts.
...
The key point to understand is that we haven't overridden the `aspect-ratio` property computed by the browser; we've only changed the image's _rendered_ height. We can set any responsive width and height in our image's CSS while preserving the aspect ratio generated by the user-agent stylesheet.
